...
const StartNumberGame = () => {
  ...
  const handleValue = useCallback((val: any, index: number | undefined) => {
    return setInputs((inputs) => inputs.map((input, i) => (i === index ? val : input)))
  }, [])
    ...
    <div className='start-game-page container'>
      ...
      <div className='start-game-group'>
        ...
        {Array(4)
          .fill(null)
          .map((_, index) => {
            const slicedInputs = inputsCells.slice(dynamic * tab, dynamic * (tab + 1))
            return (
              <div key={index}>
                {tab === index && <div className='inputs-groups'>{slicedInputs}</div>}
              </div>
            )
          })}
      </div>

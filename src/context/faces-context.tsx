...
type PersonSetter = (
  person: PersonWithAll[] | ((person: PersonWithAll[]) => PersonWithAll[])
) => void

interface Types {
  memorizationPeople: PersonWithAll[]
  setMemorizationPeople: PersonSetter

  recallPeople: PersonWithAll[]
...
export const FacesContextProvider = ({ children }: { children: ReactNode }): JSX.Element => {
  const [memorizationPeople, setMemorizationPeople] = useState<PersonWithAll[]>([])
  const [recallPeople, setRecallPeople] = useState<PersonWithAll[]>([])
  ...
  // const recallShuffle: PersonWithAll[] = shuffle(allPeople)

  useEffect(() => {
    setMemorizationPeople(allPeople)
    setRecallPeople(shuffle(allPeople))
  }, [])

  // to check whether memorizationPeople equal to recallPeople or not
